    ...
    <body>
        ...
        <script>
            ...
            function getCreateRoute(emergency_location, position) {
                ...
                /*
            if (confirm("best place " + hospital_available[optimal_hospital.hospital_index].name + " total distance: " + optimal_hospital.distance + ", response time: " + optimal_hospital.response_time.toFixed(3) + "minutes, do you want to proceed?")) {
                //console.log("winner %o", optimal_hospital);
                hospital_available[optimal_hospital.hospital_index].available--;
                var hospital = hospital_available[optimal_hospital.hospital_index];
                var hospital_pos = setHospitalAvailable(hospital, position);
                emergency_marker[position].onGoing = true;
                emergency_marker[position].hospital_pos = hospital_pos;
                ...
                saveResponseTime(optimal_hospital,emergency_loc);
                //saveEmergencyMarker();
                //localStorage.setItem("Hospital", JSON.stringify(hospital_available[optimal_hospital.hospital_index]));


                //localStorage.setItem("Emergency", JSON.stringify(emergency_location));
                window.open('route.html?'+JSON.stringify(hospital_available[optimal_hospital.hospital_index])+"**"+JSON.stringify(emergency_location));
                initMap();
            }*/
        ...
        <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBymq4YRMhZoMwnPUd2SfyzQQLEvUtafkM&libraries=places&callback=initTrafficData" async defer></script>
    </body>
